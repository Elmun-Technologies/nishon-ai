...
export function runRulesEngine(
  ...
  const winners = sortedByRoas
    .filter(
      (a) =>
        a.roas >= (goal?.targetRoas ?? THRESHOLDS.roas.warning) &&
        a.impressions > 200,
    )
    .slice(0, 3)
    .map((a) => a.adId);
  ...
  const losers = sortedByRoas
    ...
    .filter(
      (a) =>
        a.roas < THRESHOLDS.roas.critical || a.ctr < THRESHOLDS.ctr.critical,
    )
    .slice(-3)
    .map((a) => a.adId);
...
function checkHookRate(ad: AdPerformance, problems: DetectedProblem[]): void {
  ...
  if (ad.hookRate < THRESHOLDS.hookRate.critical) {
    problems.push({
      ...
      severity: "critical",
      value: ad.hookRate,
      threshold: THRESHOLDS.hookRate.critical,
    ...
    problems.push({
      ...
      severity: "warning",
      value: ad.hookRate,
      threshold: THRESHOLDS.hookRate.warning,
...
function checkWastedSpend(
  ...
  if (ad.spend >= THRESHOLDS.wastedSpend.minSpend && ad.conversions === 0) {
    problems.push({
      ...
      severity: "warning",
      value: ad.spend,
      threshold: THRESHOLDS.wastedSpend.minSpend,
...
function checkBudgetConcentration(
  ...
  if (
    share >= THRESHOLDS.budgetConcentration.threshold &&
    ...
    opportunities.push({
      ...
      potentialImpact:
        "More even budget split could reveal higher-performing audiences",
    });
  }
}
...
function checkScaleOpportunity(
  ...
  const isWinner =
    ad.roas >= THRESHOLDS.scaleOpportunity.minRoas && ad.roas > avgRoas * 1.5;
...
function computeConfidence(campaign: CampaignPerformance): number {
  ...
  if (impressions >= THRESHOLDS.minimumImpressions.limited) return 0.9;
  if (impressions >= THRESHOLDS.minimumImpressions.insufficient) return 0.6;
  return 0.3;
}
...
function computeDataQuality(
  ...
  if (impressions >= THRESHOLDS.minimumImpressions.insufficient)
    return "limited";
  return "insufficient";
}
