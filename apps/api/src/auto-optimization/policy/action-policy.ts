...
export function governAction(
  ...
  if (
    policy.protectedCampaignIds.includes(action.targetId) ||
  ...
  if (
    constraints?.doNotPause?.includes(action.targetId) &&
  ...
  if (riskLevel === "high") {
    if (action.type === "pause_creative" && policy.allowAutoPauseCreative) {
      ...
      return approved(
        action,
        riskLevel,
        "Workspace policy allows automatic creative pausing.",
      );
    }
  ...
  if (riskLevel === "medium") {
    if (
      (action.type === "broaden_audience" ||
        action.type === "narrow_audience") &&
    ...
    if (
      (action.type === "shift_budget" || action.type === "duplicate_winner") &&
      !policy.allowAutoBudgetChange
    ) {
      return requireApproval(
        action,
    ...
    return approved(
      action,
      riskLevel,
      "Medium-risk action — within policy limits.",
    );
  }
  ...
  if (
    ...
    action.type === "rotate_creative" ||
    action.type === "rewrite_headline" ||
    action.type === "rewrite_primary_text" ||
  ...
  return approved(
    action,
    riskLevel,
    "Low-risk content action — auto-apply permitted.",
  );
}
...
export function governActions(
  ...
  const summary: GovernanceSummary = {
    ...
    autoApply: governed.filter((g) => g.governance === "AUTO_APPLY_ALLOWED")
      .length,
    approvalRequired: governed.filter(
      (g) => g.governance === "APPROVAL_REQUIRED",
    ).length,
...
function highRiskReason(type: ActionType): string {
  const reasons: Partial<Record<ActionType, string>> = {
    pause_creative:
      "Pausing a creative directly affects running campaigns — human review required.",
    pause_adset:
      "Pausing an ad set stops delivery and may impact revenue — human review required.",
    increase_budget:
      "Automatic budget increases require explicit approval to prevent overspend.",
    decrease_budget:
      "Budget reductions may hurt campaign performance — human review required.",
  };
  return (
    reasons[type] ??
    "High-risk action — human approval required by default policy."
  );
}
