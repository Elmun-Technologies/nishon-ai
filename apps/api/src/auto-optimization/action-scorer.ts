...
function scoreAction(
  ...
  const score = Math.min(
    100,
    Math.round(priorityScore + confidenceScore + effortScore),
  );
...
function computeConfidence(
  ...
  const corroborated = ruleAnalysis.problems.some(
    (p) => p.targetId === action.targetId,
  );
  const bonus = corroborated ? 0.1 : 0;

  // Winners get a confidence boost on scale/duplicate actions
  const isWinnerAction = [
    "increase_budget",
    "duplicate_winner",
    "shift_budget",
...
function effortInverseWeight(effort: "low" | "medium" | "high"): number {
  switch (effort) {
    case "low":
      return 1.0; // easy win — boost score
    case "medium":
      return 0.65;
    case "high":
      return 0.3; // high effort actions score lower unless priority is critical
  }
}
