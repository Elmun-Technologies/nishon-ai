...
export class CommissionCalculationService {
   ...
   */
  async calculateCommissionForDeal(
    dealId: string,
    ...
    try {
      ...
      const saved = await this.specialistCommissionRepository.save(
        commission as any,
      );
      ...
      this.logger.log(
        `Commission calculated for deal ${dealId}: $${finalCommissionAmount}`,
      );
      return saved as any;
    } catch (error) {
    ...
    try {
      ...
      for (const deal of closedInMonth) {
        try {
          ...
          this.logger.error(
            `Failed to calculate commission for deal ${deal.id}: ${error.message}`,
          );
        }
      }
      ...
      this.logger.error(
        `Failed to calculate monthly commissions: ${error.message}`,
      );
      throw error;
    }
  }
    ...
    try {
      ...
      if (rateOverride !== undefined) {
        ...
        commission.commissionAmount =
          (commission.dealValue * rateOverride) / 100;
      } else {
        // Use current rate
        const rate = await this.commissionRateService.getRate(
      ...
      const updated =
        await this.specialistCommissionRepository.save(commission);
