...
import { LinkedDeal, RevenueSyncLog, CampaignRevenue } from "../entities";
import {
  AmoCRMDeal,
...
export class DealPullSyncService {
    ...
    try {
      ...
      for (const deal of deals) {
        try {
          ...
          this.logger.error(
            `Failed to process deal ${deal.id}: ${error.message}`,
          );
          dealsFailed++;
        }
      }
      ...
      // 4. Log sync results
      syncLog.status =
        dealsFailed > 0 ? SyncStatus.PARTIAL : SyncStatus.SUCCESS;
  ...
  private async processDeal(
    ...
    if (!linkedDeal) {
      ...
      linkedDeal.wonAt = deal.closed_at
        ? new Date(deal.closed_at * 1000)
        : null;
    }
    ...
    if (campaignId) {
      ...
      linkedDeal.platform =
        (platform as "google" | "meta" | "tiktok" | "yandex" | null) || null;
    ...
    try {
      ...
      this.logger.warn(
        `Failed to update audience segments for deal: ${error.message}`,
      );
