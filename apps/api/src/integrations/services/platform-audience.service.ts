...
export class PlatformAudienceService {
  ...
  async createAudience(
    ...
    try {
      ...
      this.logger.error(
        `Failed to create audience on ${platform}: ${error.message}`,
      );
      throw error;
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < members.length; i += batchSize) {
        ...
        try {
          switch (platform) {
            case "meta":
              const metaResult = await this.addMetaMembers(
                audienceId,
                batch,
                connectionId,
              );
              ...
              break;
            case "google":
              const googleResult = await this.addGoogleMembers(
                audienceId,
                batch,
                connectionId,
              );
              ...
              break;
            case "tiktok":
              const tikTokResult = await this.addTikTokMembers(
                audienceId,
                batch,
                connectionId,
              );
              ...
              break;
            case "yandex":
              const yandexResult = await this.addYandexMembers(
                audienceId,
                batch,
                connectionId,
              );
    ...
    try {
      ...
      for (let i = 0; i < members.length; i += batchSize) {
        ...
        try {
          switch (platform) {
            case "meta":
              const metaResult = await this.removeMetaMembers(
                audienceId,
                batch,
                connectionId,
              );
      ...
      this.logger.error(
        `Failed to remove members from audience: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async updateAudienceMetadata(
    ...
    try {
      switch (platform) {
        case "meta":
          await this.updateMetaAudienceMetadata(
            audienceId,
            metadata,
            connectionId,
          );
          break;
        case "google":
          await this.updateGoogleAudienceMetadata(
            audienceId,
            metadata,
            connectionId,
          );
  ...
  private async createMetaAudience(
    _config: any,
  ): Promise<{ audienceId: string; url: string }> {
  ...
  private async createGoogleAudience(
    _config: any,
  ): Promise<{ audienceId: string; url: string }> {
  ...
  private async createTikTokAudience(
    _config: any,
  ): Promise<{ audienceId: string; url: string }> {
  ...
  private async createYandexAudience(
    _config: any,
  ): Promise<{ audienceId: string; url: string }> {
