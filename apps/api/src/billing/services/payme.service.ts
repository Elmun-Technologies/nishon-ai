...
import {
  PaymeTransaction,
  PaymeTransactionState as _PaymeTransactionState,
} from "../entities/payme-transaction.entity";
...
export class PaymeService {
  ...
  constructor(
    ...
    if (!this.merchantId || !this.merchantKey) {
      this.logger.warn(
        "PAYME_MERCHANT_ID or PAYME_MERCHANT_KEY not set — Payme integration disabled",
      );
    }
  }
  ...
  private async createTransaction(params: Record<string, any>) {
    ...
    if (existing) {
      ...
      // Check timeout
      if (
        Date.now() - Number(existing.paymeCreateTime) >
  ...
  private async performTransaction(params: Record<string, any>) {
    ...
    this.logger.log(
      `Payment completed: order=${tx.orderId}, plan=${tx.targetPlan}, amount=${tiyinToUzs(Number(tx.amount))} UZS`,
    );
  ...
  private async cancelTransaction(params: Record<string, any>) {
    ...
    if (tx.state === 2) {
      ...
      this.logger.warn(
        `Refund processed: order=${tx.orderId}, user downgraded to FREE`,
      );
  ...
  async createOrder(
    ...
    this.logger.log(
      `Order created: ${orderId}, plan=${targetPlan}, amount=${tiyinToUzs(amountTiyin)} UZS`,
    );
  ...
  async getOrderStatus(orderId: string) {
    ...
    return {
      ...
      paidAt: tx.paymePerformTime
        ? new Date(Number(tx.paymePerformTime))
        : null,
    };
  }
