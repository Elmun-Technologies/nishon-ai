...
export class MetaConnector {
  ...
  async createCampaign(
    ...
    this.logger.log(`Creating Meta campaign: ${params.name} for account: ${adAccountId}`)
  ...
  async updateCampaignBudget(
    ...
    this.logger.log(`Meta campaign budget updated: ${campaignId} → $${dailyBudgetUsd}`)
  }
  ...
  async searchAdLibrary(params: {
    ...
    const data = await this.apiGet<{ data: any[] }>(
      `${META_BASE_URL}/ads_archive`,
      {
        ...
        fields: 'id,ad_creative_body,ad_creative_link_caption,page_name,ad_delivery_start_time,impressions,spend',
  ...
  private async apiGet<T>(url: string, params: Record<string, any>): Promise<T> {
    try {
      ...
      this.logger.error(`Meta API GET error: ${metaError?.message || error.message}`)
  ...
  private async apiPost<T>(url: string, data: Record<string, any>): Promise<T> {
    try {
      ...
      const metaError = error.response?.data?.error
      this.logger.error(`Meta API POST error: ${metaError?.message || error.message}`)
      throw new BadRequestException(
        metaError?.message || 'Meta API request failed',
      )
    }
