...
describe("PlatformsService — workspace ownership", () => {
  ...
  beforeEach(async () => {
    ...
    accountRepo = {
      ...
      findOne: jest.fn().mockResolvedValue({ id: "acc-1", workspaceId: "ws-1" }),
      remove: jest.fn().mockResolvedValue(undefined),
    };
    metaConnector = { getPages: jest.fn().mockResolvedValue([{ id: "p1", name: "Page" }]) };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ConfigService, useValue: { get: jest.fn(() => "0".repeat(32)) } },
      ],
    }).compile();
  ...
  describe("getConnectedAccounts", () => {
    it("returns accounts for the workspace owner", async () => {
      ...
      await expect(service.getConnectedAccounts("ws-1", OWNER)).resolves.toEqual([]);
      expect(accountRepo.find).toHaveBeenCalled();
    });

    it("denies a non-owner (IDOR)", async () => {
      workspaceRepo.findOne.mockResolvedValue({ id: "ws-1", userId: OWNER });
      await expect(service.getConnectedAccounts("ws-1", ATTACKER)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
    ...
    it("404s for a missing workspace", async () => {
      ...
      await expect(service.getConnectedAccounts("ghost", OWNER)).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
  });
  ...
  describe("getMetaPages", () => {
    it("denies a non-owner before touching the Meta API", async () => {
      workspaceRepo.findOne.mockResolvedValue({ id: "ws-1", userId: OWNER });
      await expect(service.getMetaPages("ws-1", ATTACKER)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
