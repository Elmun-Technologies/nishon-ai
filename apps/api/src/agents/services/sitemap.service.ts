...
export class SitemapService {
  ...
  async generateAllSitemaps(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Successfully generated sitemaps with ${urls.length} URLs`,
      );
    } catch (error) {
  ...
  private async generateSitemaps(urls: SitemapUrl[]): Promise<void> {
    ...
    for (let i = 0; i < sitemapCount; i++) {
      ...
      const filename =
        sitemapCount === 1 ? "sitemap.xml" : `sitemap-${i + 1}.xml`;
  ...
  private async generateSitemapIndex(): Promise<void> {
    ...
    if (files.length === 0) {
      fs.writeFileSync(
        path.join(this.sitemapDir, "sitemap.xml"),
        indexXml,
        "utf-8",
      );
    }
