...
`;
  ...
  validateRobotsTxt(content: string): {
    ...
    for (const line of lines) {
      ...
      if (!keyLower || !value) {
        errors.push(
          `Line ${lineNumber}: Invalid format. Expected "key: value"`,
        );
        continue;
      }
      ...
      if (keyLower === "disallow" || keyLower === "allow") {
        if (!value.startsWith("/") && value !== "*") {
          errors.push(
            `Line ${lineNumber}: ${keyLower} path should start with /`,
          );
        }
      }
  ...
  async getRobotsStats(): Promise<{
    ...
    const lines = content.split("\n");
    const userAgentCount = lines.filter((l) =>
      l.toLowerCase().startsWith("user-agent:"),
    ).length;
    const disallowCount = lines.filter((l) =>
      l.toLowerCase().startsWith("disallow:"),
    ).length;
    const allowCount = lines.filter((l) =>
      l.toLowerCase().startsWith("allow:"),
    ).length;
