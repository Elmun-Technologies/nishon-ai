...
export class CreativeService {
  ...
  async generateImageCreative(
    ...
    if (!this.openai) {
      throw new Error(
        "Image generation unavailable: OPENAI_API_KEY is not configured",
      );
    }
    ...
    this.logger.log(
      `Generating image creative: size=${size}, style=${dto.stylePreset}`,
    );
  ...
  async generateVideoCreative(
    ...
    if (!this.higgsFieldApiKey) {
      throw new Error(
        "Video generation unavailable: HIGGSFIELD_API_KEY is not configured",
      );
    }

    this.logger.log(
      `Generating video creative: avatar=${dto.avatarStyle}, duration=${dto.duration}s`,
    );
    ...
    if (!submitRes.ok) {
      ...
      throw new Error(
        `Higgsfield API error ${submitRes.status}: ${errText.slice(0, 300)}`,
      );
    }
  ...
  private async pollHiggsFieldJob(jobId: string): Promise<string> {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      if (data.status === "failed") {
        throw new Error(
          `Higgsfield job failed: ${data.error ?? "unknown error"}`,
        );
      }
    }
  ...
  async generateTextToImage(
    ...
    if (!this.openai) {
      throw new Error(
        "Text-to-image unavailable: OPENAI_API_KEY is not configured",
      );
    }
    ...
    this.logger.log(
      `Generating text-to-image: style=${dto.artStyle}, quality=${quality}`,
    );
