...
import { ArrowLeft, ArrowRight, Inbox, Loader2, Search } from 'lucide-react'
import { Button } from '@/components/ui'
import { useI18n } from '@/i18n/use-i18n'
import { cn } from '@/lib/utils'
import type { AdLauncherController } from '../_lib/use-ad-launcher'
import type { CampaignRow } from '../_lib/types'
...
export function PickStep({ ctl }: { ctl: AdLauncherController }) {
  ...
  const allChecked = useMemo(
    () => ctl.filteredCampaigns.length > 0 && ctl.filteredCampaigns.every((c) => ctl.selectedIds.has(c.id)),
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 shadow-sm">
        ...
        <span className="text-xs text-text-tertiary">
          {t('adLauncher.foundCount', '{{n}} ta topildi').replace('{{n}}', String(ctl.filteredCampaigns.length))}
        </span>
      </div>
        ...
        <table className="min-w-[820px] w-full border-collapse text-left text-sm">
          <thead className="border-b border-border bg-surface-2 text-xs uppercase tracking-wide text-text-tertiary dark:bg-surface-elevated">
            <tr>
              ...
              <th className="px-3 py-2.5">{t('adLauncher.colName', 'Reklama nomi')}</th>
              <th className="px-3 py-2.5">{t('adLauncher.colStatus', 'Holat')}</th>
              <th className="px-3 py-2.5 text-right">{t('adLauncher.colSpend', 'Sarf')}</th>
              <th className="px-3 py-2.5 text-right">{t('adLauncher.colClicks', 'Bosishlar')}</th>
              <th className="px-3 py-2.5 text-right">{t('adLauncher.colCtr', 'CTR')}</th>
              <th className="px-3 py-2.5 text-right">{t('adLauncher.colCpc', 'CPC')}</th>
              <th className="px-3 py-2.5">{t('adLauncher.colAiHealth', 'AI baho')}</th>
            </tr>
          </thead>
          <tbody>
            {ctl.loading ? (
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center text-text-tertiary">
                  ...
                  <p className="mt-2 text-sm">{t('adLauncher.loadingCampaigns', 'Reklamalar yuklanmoqda...')}</p>
                </td>
              </tr>
            ) : ctl.filteredCampaigns.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center">
                  <Inbox className="mx-auto h-8 w-8 text-text-tertiary" />
                    ...
                    <td className="px-3 py-2.5">
                      <span
                        className={cn(
                          ...
                          STATUS_STYLE[row.status] ?? 'bg-slate-500/12 text-slate-700 dark:text-slate-300',
                        )}
                      >
                        {row.status}
                      </span>
