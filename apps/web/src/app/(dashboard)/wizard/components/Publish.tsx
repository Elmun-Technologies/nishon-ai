...
export function Publish({ 
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-xl font-semibold text-white">Publish Campaign</h2>
      ...
      <Card padding="lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <p className="text-white font-medium">{formData.objective}</p>
          </div>
          <div>
            <Label className="text-sm text-[#6B7280]">Budget</Label>
            <p className="text-white font-medium">{formData.budget.amount} {formData.budget.currency} ({formData.budget.type})</p>
          </div>
          <div>
      ...
      {/* Ad Groups Summary */}
      <Card padding="lg">
        <h3 className="text-lg font-semibold text-white mb-4">Ad Groups</h3>
        <div className="space-y-4">
          {formData.adGroups.map((adGroup: any, index: number) => (
            <div key={index} className="border border-[#3A3A4A] rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-white font-medium">{adGroup.name}</h4>
                <Badge variant="secondary">{adGroup.scenario}</Badge>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  <span className="text-[#6B7280]">Keywords:</span>
                  <span className="text-white ml-2">{adGroup.keywords.length}</span>
                </div>
                <div>
                  <span className="text-[#6B7280]">Locations:</span>
                  <span className="text-white ml-2">{adGroup.geoTargeting.locations?.length || 0}</span>
                </div>
                <div>
                  <span className="text-[#6B7280]">Audience Segments:</span>
                  <span className="text-white ml-2">{adGroup.audienceSegments?.length || 0}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <Card padding="lg">
        <h3 className="text-lg font-semibold text-white mb-4">Creative Assets</h3>
          ...
          <div>
            ...
            <div className="space-y-1 mt-2">
              {formData.creatives.headlines.map((headline: string, index: number) => (
                <div key={index} className="text-white text-sm bg-[#2A2A3A] p-2 rounded">
                  {headline}
                </div>
              ))}
            </div>
          ...
          <div className="mt-4">
            ...
            <div className="mt-2">
              <img 
                ...
                className="max-w-md h-auto rounded-lg border border-[#3A3A4A]"
              />
            </div>
          </div>
        )}
        ...
        <Card padding="lg">
          ...
          <div className="space-y-4">
            {selectedPlatforms.map(platform => {
              ...
              return (
                <div key={platform} className="border border-[#3A3A4A] rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
      ...
      <Card padding="lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="space-y-2">
            <h4 className="text-white font-medium">Platform Review</h4>
          ...
          <div className="space-y-2">
            <h4 className="text-white font-medium">Campaign Activation</h4>
            <p className="text-sm text-[#6B7280]">
