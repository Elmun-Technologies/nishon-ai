...
export default function CreateAgentPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-text-primary">AI Agent yaratish</h1>
        ...
        {[1, 2, 3].map(s => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
              step > s ? 'bg-emerald-500 text-white' : step === s ? 'bg-surface text-white' : 'bg-surface-2 text-text-tertiary'
            }`}>
              {step > s ? '✓' : s}
            </div>
            ...
            {s < 3 && <div className="w-8 h-px bg-surface-2" />}
          </div>
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="bg-surface border border-border rounded-2xl p-6 space-y-5">
          ...
          {/* Avatar selector */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-3">Agent belgisi</label>
              ...
              {COLOR_OPTIONS.map(c => (
                <button
                  ...
                  className={`w-7 h-7 rounded-full bg-gradient-to-br ${c.value} border-2 transition-all ${
                    form.avatarColor === c.value ? 'border-border scale-110' : 'border-transparent'
                  }`}
                  title={c.label}
                />
              ))}
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-text-primary">{form.displayName || 'Agent nomi'}</p>
                <p className="text-xs text-text-tertiary">{form.title || 'Sarlavha'}</p>
              </div>
            </div>
          </div>

          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Agent nomi *</label>
            <input
              value={form.displayName}
              onChange={e => setForm(f => ({ ...f, displayName: e.target.value }))}
              placeholder="Masalan: Meta Pro AI, E-commerce Optimizer"
              className="w-full bg-surface-2 border border-border rounded-lg px-4 py-3 text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-border/50"
            />
          </div>

          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Sarlavha *</label>
            <input
              value={form.title}
          ...
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Tavsif</label>
            <textarea
              value={form.bio}
              onChange={e => setForm(f => ({ ...f, bio: e.target.value }))}
              placeholder="Bu agent nima qiladi, qanday natijalarga erishadi..."
              rows={3}
              className="w-full bg-surface-2 border border-border rounded-lg px-4 py-3 text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-border/50 resize-none"
            />
          </div>

          {/* Platforms */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Platformalar *</label>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                    form.platforms.includes(p)
                      ...
                      : 'bg-surface-2 text-text-tertiary border-border hover:border-border'
                  }`}
                >
                  {p}
                </button>
          ...
          {/* Niches */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Niche ixtisoslashuv</label>
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                className="flex-1 bg-surface-2 border border-border rounded-lg px-3 py-2 text-xs text-text-primary placeholder:text-text-tertiary focus:outline-none"
              />
            </div>
            {form.niches.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {form.niches.map(n => (
                  <span key={n} className="flex items-center gap-1 bg-surface text-white text-xs px-2.5 py-1 rounded-lg">
                    {n}
          ...
          {/* Pricing */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Narx modeli</label>
              ...
              {(form.pricingModel === 'fixed' || form.pricingModel === 'hybrid') && (
                <div className="flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    placeholder="49"
                    className="w-24 bg-surface-2 border border-border rounded-lg px-3 py-2 text-xs text-text-primary focus:outline-none"
                  />
                  <span className="text-xs text-text-tertiary">/oy</span>
                </div>
              )}
              {(form.pricingModel === 'commission' || form.pricingModel === 'hybrid') && (
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    placeholder="8"
                    className="w-20 bg-surface-2 border border-border rounded-lg px-3 py-2 text-xs text-text-primary focus:outline-none"
                  />
                  <span className="text-xs text-text-tertiary">% komissiya</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <button
              ...
              className="bg-surface hover:bg-surface disabled:opacity-40 text-white px-6 py-3 rounded-xl font-semibold transition-all"
            >
              Davom etish →
            </button>
          </div>
      ...
      {step === 2 && (
        <div className="bg-surface border border-border rounded-2xl p-6 space-y-5">
          <div>
            <h2 className="text-lg font-bold text-text-primary mb-1">AI Konfiguratsiya</h2>
            <p className="text-sm text-text-tertiary">Agent qanday ishlashini sozlang</p>
          </div>

          {/* Autopilot mode */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-3">Ish rejimi</label>
          ...
          {/* Min budget */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Minimal byudjet ($/oy)</label>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-32 bg-surface-2 border border-border rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none"
              />
              <span className="text-xs text-text-tertiary">Bu summadan past byudjetlarda agent ishlamaydi</span>
            </div>
          </div>

          {/* Decision frequency */}
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Qaror qabul qilish chastotasi</label>
          ...
          <div>
            <label className="text-xs font-semibold text-text-tertiary uppercase tracking-wider block mb-2">Qo'llab-quvvatlanadigan platformalar</label>
            <p className="text-xs text-text-tertiary mb-2">Standart bo'yicha — barcha tanlangan platformalar</p>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="bg-surface-2 border border-border rounded-xl p-4">
            <p className="text-xs font-semibold text-text-tertiary mb-2">Agent xulosasi:</p>
            <div className="flex items-center gap-3 mb-2">
              <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${form.avatarColor} flex items-center justify-center text-xl`}>
                {form.avatar}
              </div>
              <div>
                <p className="font-semibold text-sm text-text-primary">{form.displayName}</p>
                <p className="text-xs text-text-tertiary">{form.title}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs text-text-tertiary">
              <span>Mode: <b className="text-text-primary">{form.autopilotMode === 'FULL_AUTO' ? 'To\'liq avtomatik' : 'Yarim avtomatik'}</b></span>
              <span>Minimal: <b className="text-text-primary">${form.minManagedBudget}/oy</b></span>
              <span>Qarorlar: <b className="text-text-primary">har {form.decisionFrequencyHours}h</b></span>
              <span>Narx: <b className="text-text-primary">
          ...
          <div className="flex justify-between">
            <button onClick={() => setStep(1)} className="text-text-tertiary hover:text-text-primary text-sm transition-colors">
              ← Orqaga
            </button>
            <button
              onClick={handleCreate}
              disabled={saving}
              className="bg-surface hover:bg-surface disabled:opacity-40 text-white px-6 py-3 rounded-xl font-semibold transition-all flex items-center gap-2"
            >
      ...
      {step === 3 && createdAgent && (
        <div className="bg-surface border border-border rounded-2xl p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-text-primary mb-2">Agent yaratildi!</h2>
          <p className="text-text-tertiary text-sm mb-6">
            Agentingiz tayyor. Nashr qilish uchun moderatsiyadan o'tishi kerak.
          </p>

          <div className="bg-surface-2 border border-border rounded-xl p-4 mb-6 flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${form.avatarColor} flex items-center justify-center text-2xl`}>
              {form.avatar}
            </div>
