  ...
  return (
    <div className="space-y-5 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Top Ads</h1>
          <p className="text-text-tertiary text-sm">See your top performing ads and campaigns by CTR</p>
        </div>
      ...
      <Card padding="none">
        {loading ? (
          ...
          <div className="p-12 text-center space-y-3">
            <p className="text-2xl font-bold text-text-primary">No landing page data found</p>
            <p className="text-sm text-text-tertiary">Launch campaigns with landing pages to see your performance data here.</p>
              ...
              <thead>
                <tr className="border-b border-border">
                  {['Campaign', 'Status', 'Spend', 'Clicks', 'Impressions', 'CTR'].map((h) => (
                    <th key={h} className="text-left px-4 py-3 text-xs text-text-tertiary uppercase">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.campaignId} className="border-b border-border">
                    <td className="px-4 py-3 text-sm text-text-primary">{r.name}</td>
                    <td className="px-4 py-3 text-sm text-text-tertiary">{r.status}</td>
