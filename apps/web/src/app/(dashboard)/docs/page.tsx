      ...
      <div className="bg-surface border border-border rounded-2xl p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <input
            ...
            placeholder="Search..."
            className="ml-auto border border-border rounded-xl px-3 py-2 text-sm min-w-64"
          />
        </div>
        <div className="flex gap-4 flex-wrap border-b border-border pb-2">
          {TOP_TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`text-sm font-medium pb-1 border-b-2 ${
                tab === t.id ? 'text-[#4F46E5] border-[#4F46E5]' : 'text-text-tertiary border-transparent'
              }`}
            >
              {t.label}
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr_220px] gap-4">
        ...
        <div className="bg-surface border border-border rounded-xl p-6 space-y-6">
          <div>
            ...
            <h2 className="text-4xl font-bold text-text-primary">{current.heading}</h2>
            <p className="text-text-secondary mt-3">{current.intro}</p>
          </div>

          {filteredSections.map((section) => (
            <section id={section.id} key={section.id} className="border border-border rounded-xl p-4">
              <h3 className="text-2xl font-semibold text-text-primary mb-2">{section.title}</h3>
              <p className="text-text-secondary">{section.body}</p>
              {section.steps && (
                <ol className="mt-3 space-y-1 list-decimal list-inside text-sm text-text-secondary">
        ...
        <div className="bg-surface border border-border rounded-xl p-4 h-fit">
          <p className="text-sm font-semibold text-text-primary mb-2">On this page</p>
          <div className="space-y-1">
            {filteredSections.map((s) => (
              <a key={s.id} href={`#${s.id}`} className="block text-sm text-text-tertiary hover:text-text-primary">
                {s.title}
              </a>
            ))}
          </div>
