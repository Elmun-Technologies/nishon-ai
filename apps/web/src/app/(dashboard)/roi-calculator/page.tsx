...
export default function RoiCalculatorPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 mb-2">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-900 dark:text-slate-50 text-sm focus:outline-none focus:border-[#111827] transition-all"
            >
              {Object.entries(INDUSTRY_DATA).map(([key, val]) => (
                <option key={key} value={key} className="bg-slate-50 dark:bg-slate-800/50">
                  {val.label}
                </option>
              ))}
            </select>
              ...
              {[1, 3, 6, 12].map((m) => (
                <button
                  ...
                  className={`py-2.5 rounded-lg border text-sm font-medium transition-all ${
                    months === m
                      ? 'border-[#111827] bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
              ...
              {[
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg border text-xs transition-all ${
                    platform === p.value
                      ? 'border-[#111827] bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-50'
                      : 'border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:border-slate-600'
                  }`}
                >
      ...
      <Card padding="none">
        {/* Table header */}
        <div className="grid grid-cols-3 gap-4 px-5 py-3 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 rounded-t-xl">
