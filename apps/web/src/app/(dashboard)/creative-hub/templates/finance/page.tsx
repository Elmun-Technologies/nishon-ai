...
const templates = Array.from({ length: 12 }, (_, i) => ({
  id: i + 1,
  title: `Finance Template ${i + 1}`,
  type: ['Investment', 'Crypto', 'Banking', 'Trading'][i % 4],
}))
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold text-text-primary mt-4">Finance & Crypto</h1>
        <p className="text-text-secondary mt-2">
          Professional financial service ad templates.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {templates.map((template) => (
          <Link
            key={template.id}
            href={`/creative-hub/templates/finance/${template.id}`}
          ...
          >
            {/* Placeholder Image */}
            <div className="w-full h-full bg-gradient-to-br from-warning/20 to-success/20 flex items-center justify-center">
              <div className="text-center">
                <TrendingUp size={32} className="text-text-tertiary mx-auto mb-2" />
