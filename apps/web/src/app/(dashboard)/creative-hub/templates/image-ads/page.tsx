...
const templates = Array.from({ length: 12 }, (_, i) => ({
  id: i + 1,
  title: `Image Template ${i + 1}`,
  style: ['Minimalist', 'Bold', 'Elegant', 'Modern'][i % 4],
}))

export default function ImageAdsTemplatesPage() {
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold text-text-primary mt-4">Image Ads</h1>
        <p className="text-text-secondary mt-2">
          Browse high-impact image ad templates and recreate them for your brand in seconds.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {templates.map((template) => (
          <Link
            key={template.id}
            href={`/creative-hub/templates/image-ads/${template.id}`}
          ...
          >
            {/* Placeholder Image */}
            <div className="w-full h-full bg-gradient-to-br from-info/20 to-success/20 flex items-center justify-center">
