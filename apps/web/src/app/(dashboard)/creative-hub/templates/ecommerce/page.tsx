...
const templates = Array.from({ length: 12 }, (_, i) => ({
  id: i + 1,
  title: `E-Commerce Template ${i + 1}`,
  type: ['Product Showcase', 'Sale', 'Bundle', 'Limited Time'][i % 4],
}))
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold text-text-primary mt-4">E-Commerce</h1>
        <p className="text-text-secondary mt-2">
          Shop-ready ad templates to boost your product sales.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {templates.map((template) => (
          <Link
            key={template.id}
            href={`/creative-hub/templates/ecommerce/${template.id}`}
          ...
          >
            {/* Placeholder Image */}
            <div className="w-full h-full bg-gradient-to-br from-success/20 to-info/20 flex items-center justify-center">
              <div className="text-center">
                <ShoppingCart size={32} className="text-text-tertiary mx-auto mb-2" />
