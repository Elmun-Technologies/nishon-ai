...
function StatPill({ icon: Icon, label, value, accent = false }: { icon: any; label: string; value: string; accent?: boolean }) {
  return (
    <GlassCard className="px-5 py-4 flex items-center gap-3">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${accent ? 'bg-cyan-400/20 text-cyan-600' : 'bg-gray-100/80 text-gray-600'}`}>
        <Icon size={20} />
      </div>
      <div>
    ...
    <div className="space-y-3">
      {data.map(d => (
        <div key={d.month} className="flex items-center gap-3">
          <span className="text-gray-400 text-xs w-8 flex-shrink-0 font-medium">{d.month}</span>
          <div className="flex-1 bg-gray-100/60 rounded-full h-7 overflow-hidden">
            <div
          ...
          <span className="text-gray-400 text-xs w-16 text-right flex-shrink-0">{formatSpend(d.spend)}</span>
        </div>
      ))}
    </div>
  )
      ...
      <div className="space-y-2">
        {data.map(d => (
          <div key={d.platform} className="flex items-center gap-2">
            ...
            <span className="text-gray-500 text-xs">{d.platform}</span>
            <span className="text-gray-900 text-xs font-bold ml-auto pl-4">{d.percent}%</span>
          </div>
        ))}
      </div>
    </div>
...
function ReviewCard({ r }: { r: PortfolioTargetologist['reviews'][0] }) {
  return (
    <GlassCard className="p-5" hover>
      ...
      <p className="text-gray-700 text-sm leading-relaxed italic mb-4">&ldquo;{r.text}&rdquo;</p>
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-xs font-bold text-gray-700">
          {r.author.charAt(0)}
        </div>
        <div>
          <div className="text-gray-900 text-sm font-semibold">{r.author}</div>
          <div className="text-gray-400 text-xs">{r.company}</div>
        </div>
        <span className="ml-auto text-gray-400 text-xs">{r.date}</span>
      </div>
    </GlassCard>
  )
}
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center text-center px-6">
        <div>
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Targetolog topilmadi</h2>
          <p className="text-gray-400 mb-6">Bu profil mavjud emas yoki o&apos;chirilgan</p>
          <button onClick={() => router.push('/portfolio')} className="bg-gray-900 text-white px-6 py-3 rounded-xl font-semibold hover:bg-gray-800 transition-all">
            Katalogga qaytish
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-white text-gray-900 overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl bg-white/70 border-b border-gray-200/50">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          <button onClick={() => router.push('/portfolio')} className="flex items-center gap-2 text-gray-400 hover:text-gray-900 transition-colors text-sm font-medium">
            ← Katalog
          </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => router.push('/login')}
              className="text-sm bg-gray-900 text-white px-4 py-2 rounded-xl font-semibold hover:bg-gray-800 transition-all">
              Kirish
            </button>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col lg:flex-row gap-10 items-start">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 text-base leading-relaxed max-w-2xl mb-6">{t.bio}</p>

              {/* Platform badges */}
              <div className="flex flex-wrap gap-2 mb-6">
                {t.platforms.map(p => (
                  <span key={p.id}
                    className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl bg-white/60 backdrop-blur border border-gray-200/50 text-gray-700 font-medium shadow-sm">
                    {p.icon} {p.name}
                    {p.verified && <CheckCircle2 size={12} className="text-emerald-500" />}
                    <span className="text-gray-400">({p.accountsConnected})</span>
                  </span>
                ))}
              </div>
            ...
            <GlassCard className="w-full lg:w-72 p-6 flex-shrink-0">
              <div className="flex items-center gap-1 mb-4">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={16} className={i < Math.floor(t.rating) ? 'text-amber-400 fill-amber-400' : 'text-gray-200'} />
                ))}
                <span className="text-gray-900 font-bold ml-1">{t.rating}</span>
                <span className="text-gray-400 text-xs">({t.reviewCount} sharh)</span>
              </div>

              <div className="text-gray-400 text-xs mb-1 uppercase tracking-wider font-medium">Narxi</div>
              <div className="text-gray-900 font-extrabold text-3xl mb-1">
                ${t.price.from}
                <span className="text-gray-400 text-sm font-normal">/{t.price.unit}</span>
              </div>
              ...
              <button onClick={() => router.push('/register')}
                className="w-full bg-gray-50 hover:bg-gray-100 text-gray-700 text-sm py-3 rounded-xl border border-gray-200 transition-all font-medium">
                Xizmat buyurtma berish
              </button>

              <div className="mt-5 pt-4 border-t border-gray-200/50">
                <div className="text-[10px] uppercase tracking-widest text-gray-400 font-medium mb-2">Performa kafolati</div>
      ...
      <section ref={statsRef} className="relative z-10 py-16 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-500 text-sm">Barcha ma&apos;lumotlar haqiqiy ad account lardan olingan</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <GlassCard className="lg:col-span-2 p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-gray-400 bg-gray-50 px-3 py-1 rounded-lg border border-gray-200 font-medium">Oylik</span>
              </div>
        ...
        <section className="relative z-10 py-16 px-6">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              ...
              <p className="text-gray-400 text-xs">Mijoz maxfiyligi uchun anonimlashtrilgan</p>
            </div>
                  ...
                  return (
                    <div key={c.id} className="flex items-center gap-4 py-4 px-6 hover:bg-white/60 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-gray-900 text-sm font-semibold truncate">{c.niche}</div>
                        <div className="text-gray-400 text-xs">{c.duration}</div>
                      </div>
                      <div className="text-right hidden sm:block">
                        <div className="text-gray-900 text-sm font-bold">{c.roas}x <span className="text-gray-400 font-normal">ROAS</span></div>
                        <div className="text-gray-400 text-xs">${c.spend.toLocaleString()}</div>
                      </div>
                      <div className="text-right hidden md:block">
                        <div className="text-gray-900 text-sm font-medium">${c.cpa}</div>
            ...
            <div className="text-center mb-10">
              ...
              <div className="flex items-center justify-center gap-2 mt-3">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={18} className={i < Math.floor(t.rating) ? 'text-amber-400 fill-amber-400' : 'text-gray-200'} />
                  ))}
                </div>
                <span className="text-gray-900 font-bold text-lg">{t.rating}</span>
                <span className="text-gray-400 text-sm">({t.reviewCount} sharh)</span>
              </div>
            </div>
          ...
          <GlassCard className="px-10 py-14">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button onClick={() => router.push('/portfolio')}
                className="bg-white border border-gray-200 text-gray-700 px-8 py-3.5 rounded-xl font-medium hover:bg-gray-50 transition-all">
      ...
      <footer className="relative z-10 border-t border-gray-100 py-8 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-gray-400">
          <span className="font-bold text-gray-900">Performa <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-500 to-blue-500">AI</span></span>
          <span>Barcha natijalar haqiqiy API ma&apos;lumotlari asosida tasdiqlangan</span>
          <div className="flex items-center gap-4">
            <button onClick={() => router.push('/terms')} className="hover:text-gray-900 transition-colors">Shartlar</button>
            <button onClick={() => router.push('/privacy')} className="hover:text-gray-900 transition-colors">Maxfiylik</button>
          </div>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-6">
          <GlassCard className="p-8 w-full max-w-md bg-white/90">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setContactOpen(false)} className="text-gray-400 hover:text-gray-900 text-xl transition-colors">✕</button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => router.push('/register')}
                className="w-full bg-gray-50 hover:bg-gray-100 text-gray-700 py-3 rounded-xl border border-gray-200 transition-all font-medium">
                Ro&apos;yxatdan o&apos;tish — bepul
              </button>
            </div>
            <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-center gap-4 text-xs text-gray-400">
      ...
      {/* ═══ FLOATING MOBILE CTA ═══ */}
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden backdrop-blur-xl bg-white/80 border-t border-gray-200/50 px-4 py-3">
